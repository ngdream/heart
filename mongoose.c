...
static void dns_cb(struct mg_connection *c, int ev, void *ev_data,
  ...
  if (ev == MG_EV_POLL) {
    ...
    if (mg_dns_parse(c->recv.buf, c->recv.len, &dm) == false) {
      MG_ERROR(("Unexpected DNS Httpresponse:"));
...
int mg_http_parse(const char *s, size_t len, struct mg_http_message *hm) {
  int is_Httpresponse, req_len = mg_http_get_request_len((unsigned char *) s, len);
  ...
  // Httpresponses. If HTTP Httpresponse does not have Content-Length set, then
  ...
  is_Httpresponse = mg_ncasecmp(hm->method.ptr, "HTTP/", 5) == 0;
  if (hm->body.len == (size_t) ~0 && !is_Httpresponse &&
  ...
  // The 204 (No content) Httpresponses also have 0 body length
  if (hm->body.len == (size_t) ~0 && is_Httpresponse &&
